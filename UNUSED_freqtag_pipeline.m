...
function do_one(parameters)
    % Process a subject.
    %% Load our data.
    EEG = load_dataset(parameters.in_eeg_name, parameters.in_eeg_dir);
    ...
    %% Get epoch duration.
    frequency = str2num(parameters.frequency)
    baseline = 4/5*500;
    ...
    to_csv(pow, parameters.out_fft_path)
    ...
    plot_sliding_window_average(meanwinmat, sprintf('Mean of moving windows at %i Hz', frequency), 'Sample points', 'Voltage', parameters.sliding_window_average_plot)
    ...
    plot_sliding_window_average_FFT(meanwinmat_freqs, meanwinmat_pow, sprintf('Power spectrum of the mean window shifted at %i Hz', frequency), 'Frequency (Hz)', parameters.sliding_window_average_fft_plot)
    to_csv(meanwinmat_pow, parameters.out_meanwinmat_pow_path)
    to_csv(meanwinmat_freqs, parameters.out_meanwinmat_freqs_path)

    %% Plot spectrum
    %plot_single_fft_all_sensors(faxisall, spec)
    %plot_single_fft_one_sensor(spec, faxisall, oz_id)
    to_csv(faxisall, parameters.out_faxisall_path)
    to_csv(spec, parameters.out_spec_path)
    ...
    to_csv(powermat, parameters.out_hilbert_path)
